namespace day8 {
  //input processing

  // remove extra newlines
  type trim<S extends string> = S extends `\n${infer T}`
    ? trim<T>
    : S extends `${infer T}\n`
    ? trim<T>
    : S;

  // read one line to a number array
  type readLine<
    S extends string,
    X extends number[] = []
  > = S extends `${infer H}${infer T}`
    ? H extends "\n"
      ? [X, T]
      : readLine<T, [...X, parseNum<H>]>
    : [X, S];

  // read the block to a grid
  type readLines<S extends string, X extends number[][] = []> = S extends ""
    ? X
    : readLine<S> extends [infer Y extends number[], infer R extends string]
    ? readLines<R, [...X, Y]>
    : never;

  // utils

  // create array of numbers from N-1 down to 0. useful for getting adjacent row/column indices
  // it also doubles as an easy way to make an array of size N for comparisons and the like
  type countDownFrom<
    N extends number,
    C extends number[] = []
  > = C["length"] extends N ? C : countDownFrom<N, [C["length"], ...C]>;

  // create array of numbers from A+1 up to B. useful for getting adjacent row/column indices
  type countUpTo<
    A extends number,
    B extends number,
    C extends any[] = [null, ...countDownFrom<A>],
    D extends number[] = []
  > = C["length"] extends B
    ? D extends number[]
      ? D
      : never
    : countUpTo<A, B, [null, ...C], [...D, C["length"]]>;

  // length of string
  type stringLength<
    S extends string,
    C extends null[] = []
  > = S extends `${infer _}${infer T}`
    ? stringLength<T, [null, ...C]>
    : C["length"];

  // test if strict A<B. uses arraylength compare, only useful for comparing small numbers, this is for comparing digits to build the real lessThan.
  type numericLessThan<
    A extends number,
    B extends number,
    C extends null[] = []
  > = [...C, ...countDownFrom<B>]["length"] extends A
    ? false
    : [...C, ...countDownFrom<A>]["length"] extends B
    ? true
    : numericLessThan<A, B, [null, ...C]>;

  // test if strict A<B (both are strings of equal lengths)
  type eqStrLessThan<
    A extends string,
    B extends string
  > = A extends `${infer HA extends number}${infer TA}`
    ? B extends `${infer HB extends number}${infer TB}`
      ? HA extends HB
        ? eqStrLessThan<TA, TB>
        : numericLessThan<HA, HB>
      : false
    : false;

  // test if strict A<B (both are strings)
  type strLessThan<
    A extends string,
    B extends string
  > = stringLength<A> extends infer AL extends number
    ? stringLength<B> extends infer BL extends number
      ? AL extends BL
        ? eqStrLessThan<A, B>
        : numericLessThan<AL, BL>
      : never
    : never;

  // test if strict A<B.
  type lessThan<A extends number, B extends number> = strLessThan<
    `${A}`,
    `${B}`
  >;

  // find the largest element in an array
  type maximum<N extends number[], M extends number = 0> = N extends [
    infer H extends number,
    ...infer T extends number[]
  ]
    ? maximum<T, lessThan<H, M> extends true ? M : H>
    : M;

  // take an array of arrays to an array of each array's maximum
  type mapMaximum<N extends number[][], M extends number[] = []> = N extends [
    infer H extends number[],
    ...infer T extends number[][]
  ]
    ? mapMaximum<T, [...M, maximum<H>]>
    : M;

  // we need multiplication, _sigh_

  // integer string adder (this is copypasted from day 1)

  // string to num (for use in adding individual digits)
  type parseNum<S extends string> = S extends `${infer N extends number}`
    ? N
    : never;

  // number adder
  type sum<A extends number, B extends number, C extends boolean = false> = [
    ...countDownFrom<A>,
    ...countDownFrom<B>,
    ...(C extends true ? [null] : [])
  ]["length"];

  // only call with one- or two-digit numbers - returns [digit, carry]
  type numToDigit<N> = N extends number
    ? `${N}` extends `${infer _}${infer H}${infer T}`
      ? [`${H}${T}`, true]
      : [`${N}`, false]
    : never;

  // add digit strings (1 char each)
  type addDigit<
    A extends string,
    B extends string,
    C extends boolean = false
  > = numToDigit<sum<parseNum<A>, parseNum<B>, C>>;

  // add strings in reverse order:
  type addReversed<
    A extends string,
    B extends string,
    C extends boolean = false,
    P extends string = ""
  > = A extends `${infer HA}${infer TA}`
    ? B extends `${infer HB}${infer TB}`
      ? addDigit<HA, HB, C> extends [
          infer S extends string,
          infer C2 extends boolean
        ]
        ? addReversed<TA, TB, C2, `${P}${S}`>
        : never
      : addDigit<HA, "0", C> extends [
          infer S extends string,
          infer C2 extends boolean
        ]
      ? addReversed<TA, "", C2, `${P}${S}`>
      : never
    : B extends `${infer HB}${infer TB}`
    ? addDigit<"0", HB, C> extends [
        infer S extends string,
        infer C2 extends boolean
      ]
      ? addReversed<"", TB, C2, `${P}${S}`>
      : never
    : C extends true
    ? `${P}1`
    : P;

  // reverse a string
  type reverse<
    S extends string,
    R extends string = ""
  > = S extends `${infer H}${infer T}` ? reverse<T, `${H}${R}`> : R;

  // string adder
  type addStrings<A extends string, B extends string> = reverse<
    addReversed<reverse<A>, reverse<B>>
  >;

  type addNumbers<A extends number, B extends number> = parseNum<
    addStrings<`${A}`, `${B}`>
  > extends infer P extends number
    ? P
    : never;
  // --end copypaste

  //number multiplier: A*B(+C).
  type prod<
    A extends number,
    B extends number,
    C extends number = 0,
    D extends null[] = [],
    E extends number = 0
  > = D["length"] extends B
    ? addNumbers<E, C>
    : prod<A, B, C, [null, ...D], addNumbers<A, E>>;

  // multiply two string digits. return string ones place and number tens place (carry, defaults to 0)
  type numToCarry<N> = N extends number
    ? `${N}` extends `${infer A extends number}${infer H}${infer T}`
      ? [`${H}${T}`, A]
      : [`${N}`, 0]
    : never;

  // multiply two string digits with carry, produces [ones, tens] with string ones and numeric tens
  type prodDigit<
    A extends string,
    B extends string,
    C extends number = 0
  > = numToCarry<prod<parseNum<A>, parseNum<B>, C>>;

  // multiply A, a reversed number string, by B, a single string digit. Returns a string
  type mulReversedByDigit<
    A extends string,
    B extends string,
    C extends number = 0,
    P extends string = ""
  > = A extends `${infer HA}${infer TA}`
    ? prodDigit<HA, B, C> extends [
        infer S extends string,
        infer C2 extends number
      ]
      ? mulReversedByDigit<TA, B, C2, `${S}${P}`>
      : never
    : C extends 0
    ? P
    : `${C}${P}`;

  // multiply A, a reversed number string, by B, a FORWARD number string. returns a string
  type mulReversedByString<
    A extends string,
    B extends string,
    P extends string = ""
  > = B extends `${infer BH}${infer BT}`
    ? mulReversedByString<A, BT, addStrings<`${P}0`, mulReversedByDigit<A, BH>>>
    : P;

  // fix output
  type stripLeadingZeroes<S extends string> = S extends `0${infer T}`
    ? T extends ""
      ? S
      : stripLeadingZeroes<T>
    : S;

  // multiply two strings
  type multiplyStrings<A extends string, B extends string> = stripLeadingZeroes<
    mulReversedByString<reverse<A>, B>
  >;
  // multiply two numbers
  type multiplyNumbers<A extends number, B extends number> = parseNum<
    multiplyStrings<`${A}`, `${B}`>
  >;
  // end math

  // part 1

  // view one row from the left
  type left<
    S extends number[],
    M extends number = -1,
    R extends [number, boolean][] = []
  > = S extends [infer H extends number, ...infer T extends number[]]
    ? M extends -1
      ? left<T, H, [...R, [H, true]]>
      : lessThan<M, H> extends true
      ? left<T, H, [...R, [H, true]]>
      : left<T, M, [...R, [H, false]]>
    : R;

  // view all rows from the left
  type viewLeft<
    S extends number[][],
    R extends [number, boolean][][] = []
  > = S extends [infer H extends number[], ...infer T extends number[][]]
    ? viewLeft<T, [...R, left<H>]>
    : R;

  // view one row from the right
  type right<
    S extends [number, boolean][],
    M extends number = -1,
    R extends [number, boolean][] = []
  > = S extends [
    ...infer T extends [number, boolean][],
    [infer H extends number, infer B extends boolean]
  ]
    ? M extends -1
      ? right<T, H, [[H, true], ...R]>
      : lessThan<M, H> extends true
      ? right<T, H, [[H, true], ...R]>
      : right<T, M, [[H, B], ...R]>
    : R;

  // view all rows from the right
  type viewRight<
    S extends [number, boolean][][],
    R extends [number, boolean][][] = []
  > = S extends [
    infer H extends [number, boolean][],
    ...infer T extends [number, boolean][][]
  ]
    ? viewRight<T, [...R, right<H>]>
    : R;

  // see row N from behind V, where V is the pseudo-row of the highest thing seen in each column so far.
  // V will be [] if nothing is seen yet, otherwise it will be the same length as N.
  // return [R: new row, W: new V]
  type applyViewRow<
    N extends [number, boolean][],
    V extends number[],
    R extends [number, boolean][] = [],
    W extends number[] = []
  > = N extends [
    [infer NH extends number, infer B extends boolean],
    ...infer NT extends [number, boolean][]
  ]
    ? V extends [infer VH extends number, ...infer VT extends number[]]
      ? lessThan<VH, NH> extends true
        ? applyViewRow<NT, VT, [...R, [NH, true]], [...W, NH]>
        : applyViewRow<NT, VT, [...R, [NH, B]], [...W, VH]>
      : applyViewRow<NT, V, [...R, [NH, true]], [...W, NH]>
    : [R, W];

  // view all rows from the top. return [U,V] where U is the viewed array, and V is the highest seen height in each row
  type viewTop<
    S extends [number, boolean][][],
    V extends number[] = [],
    U extends [number, boolean][][] = []
  > = S extends [
    infer H extends [number, boolean][],
    ...infer T extends [number, boolean][][]
  ]
    ? applyViewRow<H, V> extends [
        infer R extends [number, boolean][],
        infer W extends number[]
      ]
      ? viewTop<T, W, [...U, R]>
      : never
    : U;
  // view all rows from the bottom. return [U,V] where U is the viewed array, and V is the highest seen height in each row
  type viewBottom<
    S extends [number, boolean][][],
    V extends number[] = [],
    U extends [number, boolean][][] = []
  > = S extends [
    ...infer T extends [number, boolean][][],
    infer H extends [number, boolean][]
  ]
    ? applyViewRow<H, V> extends [
        infer R extends [number, boolean][],
        infer W extends number[]
      ]
      ? viewBottom<T, W, [R, ...U]>
      : never
    : U;

  // count how many are seen in a row. return null[] of that length.
  type countSeen<
    S extends [number, boolean][],
    C extends null[] = []
  > = S extends [
    [infer _, infer B extends boolean],
    ...infer T extends [number, boolean][]
  ]
    ? B extends true
      ? countSeen<T, [null, ...C]>
      : countSeen<T, C>
    : C;

  // count how many are seen in all rows. return a number.
  type countSeenRows<
    S extends [number, boolean][][],
    C extends null[] = []
  > = S extends [
    infer H extends [number, boolean][],
    ...infer T extends [number, boolean][][]
  ]
    ? countSeenRows<T, [...countSeen<H>, ...C]>
    : C["length"];

  // part 2

  // count a horizontal (left or right) component of score, given an array of column indices to check.
  type scoreHorizontal<
    B extends number[][],
    R extends number,
    C extends number,
    I extends number[],
    D extends null[] = []
  > = I extends [infer H extends number, ...infer T extends number[]]
    ? lessThan<B[R][H], B[R][C]> extends true
      ? scoreHorizontal<B, R, C, T, [null, ...D]>
      : [null, ...D]["length"]
    : D["length"];

  // check the left component of score. Uses the horizontal helper with decreasing indices starting from C.
  type scoreLeft<
    B extends number[][],
    R extends number,
    C extends number
  > = scoreHorizontal<B, R, C, countDownFrom<C>>;
  // check the right component of score. Uses the horizontal helper with increasing indices from C to width
  type scoreRight<
    B extends number[][],
    R extends number,
    C extends number
  > = scoreHorizontal<B, R, C, countUpTo<C, B[R]["length"]>>;

  // count a vertical (up or down) component of score, given an array of row indices to check
  type scoreVertical<
    B extends number[][],
    R extends number,
    C extends number,
    I extends number[],
    D extends null[] = []
  > = I extends [infer H extends number, ...infer T extends number[]]
    ? lessThan<B[H][C], B[R][C]> extends true
      ? scoreVertical<B, R, C, T, [null, ...D]>
      : [null, ...D]["length"]
    : D["length"];

  // check the up component of score. Use the vertical helper with decreasing indices starting from R
  type scoreUp<
    B extends number[][],
    R extends number,
    C extends number
  > = scoreVertical<B, R, C, countDownFrom<R>>;
  // check the down component of score. Use the vertical helper with increasing indices from R to height.
  type scoreDown<
    B extends number[][],
    R extends number,
    C extends number
  > = scoreVertical<B, R, C, countUpTo<R, B["length"]>>;

  // compute total score for a single cell
  type score<B extends number[][], R extends number, C extends number> = [
    scoreLeft<B, R, C>,
    scoreRight<B, R, C>,
    scoreUp<B, R, C>,
    scoreDown<B, R, C>
  ] extends [
    infer L extends number,
    infer M extends number,
    infer N extends number,
    infer O extends number
  ]
    ? multiplyNumbers<
        multiplyNumbers<multiplyNumbers<L, M>, N>,
        O
      > extends infer P extends number
      ? P
      : never
    : never;

  // compute scores for each cell of a row.
  type scoreRow<
    B extends number[][],
    R extends number,
    I extends number[] = countDownFrom<B[R]["length"]>,
    D extends number[] = []
  > = I extends [infer F extends number, ...infer L extends number[]]
    ? scoreRow<B, R, L, [score<B, R, F>, ...D]>
    : D;

  // compute scores for all cells
  type scoreAll<
    B extends number[][],
    I extends number[] = countDownFrom<B["length"]>,
    D extends number[][] = []
  > = I extends [infer F extends number, ...infer L extends number[]]
    ? scoreAll<B, L, [scoreRow<B, F>, ...D]>
    : D;

  // solutions

  type data<I extends string> = readLines<
    trim<I>
  > extends infer B extends number[][]
    ? B
    : never;
  type day8part1<I extends string> = countSeenRows<
    viewTop<viewBottom<viewRight<viewLeft<data<I>>>>>
  >;

  type day8part2<I extends string> = maximum<mapMaximum<scoreAll<data<I>>>>;

  type demo = `
30373
25512
65332
33549
35390
`;

  type input = `

`;

  type output = [day8part1<input>, day8part2<input>];
}
